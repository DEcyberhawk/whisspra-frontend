...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchUser = useCallback(async (token: string) => {
    try {
        ...
        await removeStorageItem('whisspra_token');
        setUser(null);
    }
  }, []);
  ...
  const handleAuthSuccess = async (data: any): Promise<boolean> => {
    if (!data.token || !data.user) {
      setLoading(false);
      return false;
    }
    await setStorageItem('whisspra_token', data.token);
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; twoFactorRequired?: boolean; tempToken?: string; }> => {
    ...
    const success = await handleAuthSuccess(data);
    return { success };
  };
