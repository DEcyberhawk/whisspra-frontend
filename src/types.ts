...
export interface BaseMessage {
    ...
    deliveryMethod?: 'server' | 'p2p' | 'mesh';
    adminAction?: {
...
export interface LiveStream {
    ...
    accessTiers: {
        ghostStream: { enabled: boolean, voiceModulation: boolean };
        greenRoom: { enabled: boolean, price: number };
        amphitheater: { enabled: boolean };
    };
    startedAt?: string;
    endedAt?: string;
}
...
export interface CognitiveAura {
    _id: string;
    creator: User;
...
export interface LiveStreamEvent {
    ...
    type: 'chat' | 'poll' | 'drop' | 'sentiment_change';
...
export interface Dreamscape {
    ...
    assetUrl: string; // URL to the 360-degree environment
    soundscapeUrl: string; // URL to the ambient audio
