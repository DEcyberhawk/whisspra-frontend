...
const PlannerTab: React.FC<PlannerTabProps> = ({ conversationId }) => {
    ...
    useEffect(() => {
        const fetchTasks = async () => {
            ...
            try {
                const res = await fetch(`${API_URL}/edu/tasks/${conversationId}`, { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const handleAddTask = async (e: React.FormEvent) => {
        ...
        const token = await getStorageItem('whisspra_token');
        try {
            const res = await fetch(`${API_URL}/edu/tasks/${conversationId}`, {
                method: 'POST',
    ...
    const handleToggleComplete = async (task: StudyTask) => {
        const token = await getStorageItem('whisspra_token');
        try {
            const res = await fetch(`${API_URL}/edu/tasks/${task._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
