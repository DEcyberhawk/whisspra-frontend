...
const navItems = [
    ...
    { name: 'Features', path: '/admin/features', icon: 'M4 21v-7M4 10V3M12 21v-9M12 8V3M20 21v-5M20 12V3M1 14h6M9 8h6M17 16h6' },
        ...
        <div className="flex h-screen bg-slate-100 dark:bg-slate-900 font-sans text-slate-800 dark:text-slate-200">
            ...
            <aside className="w-64 flex-shrink-0 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col">
                ...
                <nav className="flex-1 px-4 py-4 space-y-2">
                    {navItems.map(item => (
                        <Link 
                            key={item.name}
                            to={item.path}
                            className={`flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors duration-200 ${location.pathname.startsWith(item.path) && (item.path !== '/admin' || location.pathname === '/admin') ? 'bg-[var(--color-primary)] text-white font-semibold' : 'hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                        >
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-700">
                     <button onClick={() => navigate('/chat')} className="w-full mb-2 text-left flex items-center gap-3 px-4 py-2 text-sm text-slate-500 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg" role="menuitem">
                    ...
                    <button onClick={logout} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-red-500 dark:text-red-400 hover:bg-red-500/10 rounded-lg" role="menuitem">
